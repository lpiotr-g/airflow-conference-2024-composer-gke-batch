...
def convert_checkpoints(preset, weights_file, size, output_dir, vocab_path):
    if preset is not None:
        ...
        keras_nlp_model = keras_nlp.models.GemmaCausalLM.from_preset(keras_preset)
    ...
    for i in range(keras_nlp_model.backbone.num_layers):
        ...
        update_state_dict(
            ...
            "weight",
            torch.from_numpy(
                decoder_block.pre_attention_norm.weights[0].numpy()
            ),  # Convert to PyTorch tensor
        )

        # Attention
        query_target_shape = hf_model.model.layers[i].self_attn.q_proj.weight.shape
        query_tensor = torch.from_numpy(
            decoder_block.attention.query_dense.weights[0].numpy()
        )  # Convert to PyTorch tensor
        query_tensor = query_tensor.transpose(1, 2).reshape(
            query_target_shape
        )  # Now you can transpose and reshape
        update_state_dict(
            hf_model.model.layers[i].self_attn.q_proj, "weight", query_tensor
        )

        key_target_shape = hf_model.model.layers[i].self_attn.k_proj.weight.shape
        key_tensor = torch.from_numpy(
            decoder_block.attention.key_dense.weights[0].numpy()
        )  # Convert to PyTorch tensor
        key_tensor = key_tensor.transpose(1, 2).reshape(
            key_target_shape
        )  # Now you can transpose and reshape
        update_state_dict(
        ...
        value_target_shape = hf_model.model.layers[i].self_attn.v_proj.weight.shape
        value_tensor = torch.from_numpy(
            decoder_block.attention.value_dense.weights[0].numpy()
        )  # Convert to PyTorch tensor
        value_tensor = value_tensor.transpose(1, 2).reshape(
            value_target_shape
        )  # Now you can transpose and reshape
        update_state_dict(
        ...
        out_target_shape = hf_model.model.layers[i].self_attn.o_proj.weight.shape
        keras_out_tensor = torch.from_numpy(
            decoder_block.attention.output_dense.weights[0].numpy()
        )  # Convert to PyTorch tensor
        ...
        update_state_dict(
            ...
            "weight",
            torch.from_numpy(
                decoder_block.pre_ffw_norm.weights[0].numpy()
            ),  # Convert to PyTorch tensor
        )
        ...
        update_state_dict(
            ...
            "weight",
            torch.from_numpy(decoder_block.gating_ffw.weights[0].numpy()).transpose(
                0, 1
        ...
        update_state_dict(
            ...
            "weight",
            torch.from_numpy(decoder_block.gating_ffw_2.weights[0].numpy()).transpose(
                0, 1
            ),  # Convert and transpose
        )
        update_state_dict(
            ...
            "weight",
            torch.from_numpy(decoder_block.ffw_linear.weights[0].numpy()).transpose(
                0, 1
            ),  # Convert and transpose
        )
    ...
    update_state_dict(
        ...
        "weight",
        torch.from_numpy(
            keras_nlp_model.backbone.layers[-1].weights[0].numpy()
        ),  # Convert to PyTorch tensor
    )
...
def update_state_dict(layer, weight_name: str, tensor) -> None:
    """Updates the state dict for a weight given a tensor."""

    # Convert the Keras TensorFlow tensor to a PyTorch tensor
    if isinstance(tensor, tf.Variable):  # If tensor is a TensorFlow ResourceVariable
